          ...
          <div className="glass-strong w-full max-w-lg mx-auto rounded-t-3xl p-5 space-y-4 max-h-[80vh] overflow-hidden flex flex-col"
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Añadir algo a mano..."
                  ...
                  <div className="space-y-1">
                    ...
                    {manualShoppingItems.map(item => (
                      <div key={item.id} className="flex items-center gap-3 py-2.5 px-2" style={{ borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
                        <button onClick={() => toggleShoppingItem(item)} className={`w-4 h-4 rounded-md border flex-shrink-0 flex items-center justify-center transition-colors ${item.checked ? 'bg-[#32D74B] border-[#32D74B]' : 'border-white/20'}`}>
                          {item.checked ? <svg className="w-2.5 h-2.5 text-white" fill="none" stroke="currentColor" strokeWidth={3} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg> : null}
                        </button>
                        <span className={`flex-1 text-sm ${item.checked ? 'text-white/35 line-through' : 'text-white/80'}`}>{item.name}</span>
                        <button onClick={() => removeShoppingItem(item.id)} className="text-white/25 text-lg leading-none active:text-red-400 transition-colors">×</button>
                      </div>
                    ))}
                  </div>
                )}
