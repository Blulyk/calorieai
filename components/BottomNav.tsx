...
const NAV_RIGHT = [
  // Full-circle clock: two half-arcs (CW bottom + CW top) + hands
...
function TabBtn({ href, label, icon, active }: { href: string; label: string; icon: string; active: boolean }) {
  ...
  return (
    <Link
      ...
      style={{
        ...
        transition: pressed
          ? 'transform 0.08s ease-out, opacity 0.2s'
          : 'transform 0.35s cubic-bezier(.5,1.6,.4,1), opacity 0.2s',
    ...
    >
      ...
      <span className="text-[9.5px] font-semibold text-white" style={{ letterSpacing: '0.02em' }}>
        {label}
      </span>
    </Link>
  )
...
function FABButton({ isLog }: { isLog: boolean }) {
  const [pressed, setPressed] = useState(false)
  ...
  return (
    ...
    >
      <div style={{
        ...
        boxShadow: pressed
          ? '0 0 0 4px rgba(10,10,11,0.7), 0 2px 8px -2px rgba(10,132,255,0.4)'
          : '0 0 0 4px rgba(10,10,11,0.7), 0 0 0 5px rgba(255,255,255,0.06), 0 8px 22px -4px rgba(10,132,255,0.55), inset 0 1px 0 rgba(255,255,255,0.35)',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'center',
        transform: pressed ? 'scale(0.88)' : isLog ? 'scale(1.06)' : 'scale(1)',
        transition: pressed
          ? 'transform 0.08s ease-out, box-shadow 0.1s'
          : 'transform 0.35s cubic-bezier(.5,1.6,.4,1), box-shadow 0.25s',
      }}>
        <svg
